...
TyLog≤ {l = l} {l' = l'} f< (Bᵣ W (Bᵣ F G [ ⊢A , ⊢Π , D ] ⊢F ⊢G A≡A [Fₙ] [F] [G] G-ext)) =
  Bᵣ W (Bᵣ F G ([ Ty≤ f< ⊢A , Ty≤ f< ⊢Π , Red≤* f< D ]) (Ty≤ f< ⊢F) (Ty≤ f< ⊢G) (≅-≤ f< A≡A)
    [Fₙ]
    (λ {m} {ρ} {Δ} [ρ] ≤ε lε'' N<s ⊢Δ → [F] [ρ] (≤ₗ-• f< ≤ε) lε'' N<s ⊢Δ)
...
TyLog≤ʷ : ∀ {l l' : LCon} {lε : ⊢ₗ l} {lε' : ⊢ₗ l'} (≤ε : l ≤ₗ l') {k A}
           → ([A] :  Γ / lε ⊩⟨ k ⟩ʷ A) → Γ / lε' ⊩⟨ k ⟩ʷ A
          
TyLog≤ʷ f< (N , [A]) = (N , λ ≤ε lε'' N<s → [A] (≤ₗ-• f< ≤ε) lε'' N<s)

τTyLogʷ : ∀ {l : LCon} {lε : ⊢ₗ l} {n b nε k A}
           → ([A] :  Γ / lε ⊩⟨ k ⟩ʷ A)
           → Γ / ⊢ₗ• l lε n b nε ⊩⟨ k ⟩ʷ A
τTyLogʷ {lε = lε} {nε = nε} [A] = TyLog≤ʷ {lε = lε} {lε' = ⊢ₗ• _ lε _ _ nε}  (λ m b' mε → InThereNat _ mε _ _) [A]
...
AntiRedLog (Bᵣ W (Bᵣ F G [ ⊢B , ⊢Π , D ] ⊢F ⊢G A≡A [Fₙ] [F] [G] G-ext)) ([ ⊢A , ⊢B' , D' ]) =
  Bᵣ W (Bᵣ F G ([ ⊢A , ⊢Π , ⇒*-comp D' D ]) ⊢F ⊢G A≡A [Fₙ] (λ {m} {l'} {≤ε} → [F] {m} {l'} {≤ε}) [G] G-ext)
AntiRedLog (emb {l} {lε} {A}  0<1 [A]) D = emb 0<1 (AntiRedLog [A] D)

AntiRedLogʷ : ∀ {k A B} ([B] :  Γ / lε ⊩⟨ k ⟩ʷ B) →  Γ / lε ⊢ A :⇒*: B →  Γ / lε ⊩⟨ k ⟩ʷ A
AntiRedLogʷ (N , [B]) D = (N , λ ≤ε lε' N<s → AntiRedLog ([B] ≤ε lε' N<s) (wfRed≤* ≤ε D))
...
RedLog (Bᵣ W (Bᵣ F G [ ⊢A , ⊢Π , D ] ⊢F ⊢G A≡A [Fₙ] [F] [G] G-ext)) ([ ⊢A' , ⊢B , D' ]) =
  Bᵣ W (Bᵣ F G ([ ⊢B , ⊢Π , whrDet↘ (D , ⟦ W ⟧ₙ) D' ]) ⊢F ⊢G A≡A [Fₙ] (λ {m} {l'} {≤ε} → [F] {m} {l'} {≤ε}) [G] G-ext)
RedLog (emb {l} {lε} {A}  0<1 [A]) D = emb 0<1 (RedLog [A] D)  

RedLogʷ : ∀ {k A B} ([A] :  Γ / lε ⊩⟨ k ⟩ʷ A) →  Γ / lε ⊢ A :⇒*: B →  Γ / lε ⊩⟨ k ⟩ʷ B
RedLogʷ (N , [A]) D = (N , λ ≤ε lε' N<s → RedLog ([A] ≤ε lε' N<s) (wfRed≤* ≤ε D))
...
escape (Bᵣ′ W F G [ ⊢A , ⊢B , D ] ⊢F ⊢G A≡A [Fₙ] [F] [G] G-ext) = ⊢A
escape (emb 0<1 A) = escape A

escapeʷ-aux : ∀ {l : LCon} {lε : ⊢ₗ l} {k A} → (N : Nat)
              → ([A] : Γ / lε ⊩⟨ k ⟩ʷ A)
              → (proj₁ [A] - sizeₗ l PE.≡ N)
              → Γ / lε ⊢ A
escapeʷ-aux {l = l} 0 (M , [A]) e with ≤₋-0⇒≤ {M} {sizeₗ l} e
escapeʷ-aux {lε = lε} 0 (M , [A]) e | N<s = escape ([A] (≤ₗ-id _) lε N<s)
escapeʷ-aux {lε = lε} (1+ N) (M , [A]) e =
  ϝⱼ {nε = NewNotInLConNat _} (escapeʷ-aux N (τTyLogʷ {lε = lε} {nε = NewNotInLConNat _} (M , [A])) (-suc {n = M} e))
                             (escapeʷ-aux N (τTyLogʷ {lε = lε} {nε = NewNotInLConNat _} (M , [A])) (-suc {n = M} e))

escapeʷ : ∀ {k A} → Γ / lε ⊩⟨ k ⟩ʷ A → Γ / lε ⊢ A
escapeʷ [A] = escapeʷ-aux _ [A] PE.refl
...
reflEqAux (ℕᵣ D) D' = ⇒*-comp (red D') (red D) -- ⊩ℕ≡ _ _ (red ( [ ⊢A , ⊢ℕ , ⇒*-comp D' D ] ))
reflEqAux (𝔹ᵣ D) D' = ⇒*-comp (red D') (red D) -- ⊩𝔹≡ _ _ (red ( [ ⊢A , ⊢𝔹 , ⇒*-comp D' D ] ))
-- reflEqAux (Emptyᵣ [ ⊢B , ⊢Empty , D ]) [ ⊢A , ⊢B' , D' ] = ⇒*-comp D' D
-- reflEqAux (Unitᵣ [ ⊢B , ⊢Empty , D ]) [ ⊢A , ⊢B' , D' ] = ⇒*-comp D' D
reflEqAux (ne (ne K D neK K≡K)) D' = ne₌ K (:⇒:*-comp D' D) neK K≡K --  ne₌ _ _ [ ⊢A , ⊢K , ⇒*-comp D' D ] neK K≡K
reflEqAux (Bᵣ W (Bᵣ F G D ⊢F ⊢G A≡A [Fₙ] [F] [G] G-ext)) D' =
  (B₌ F G (⇒*-comp (red D') (red D)) A≡A [Fₙ]
      (λ [ρ] ≤ε lε' N<s N<s' ⊢Δ →
         let [[F]] = [F] [ρ] ≤ε lε' N<s ⊢Δ
         in reflEqAux [[F]] (idRed:*: (escape [[F]])))
      (λ [ρ] ≤ε lε' N<s N<s' ⊢Δ [a] → let M , [G]₂ = [G] [ρ] ≤ε lε' N<s ⊢Δ [a] in M , λ ≤ε' lε'' M<s K<s →
         let [[G]] = [G]₂ ≤ε' lε'' _ in reflEqAux [[G]] (idRed:*: (escape [[G]]))))
      -- (λ {m} {_} {_} {l'} {≤ε} {lε'} ρ Δ → reflEqAux ([F] ρ Δ) (idRed:*: (escape ([F] {≤ε = ≤ε} ρ Δ))))
      -- λ {m} {ρ} {_} {a} {l'} {≤ε} {lε'} [ρ] ⊢Δ [a] → reflEqAux ([G] [ρ] ⊢Δ [a]) (idRed:*: (escape ([G] [ρ] ⊢Δ [a])))) 
reflEqAux (emb 0<1 [A]) D = reflEqAux [A] D -- reflEqAux [A] D

reflEqAuxʷ : ∀ {k A B} ([B] :  Γ / lε ⊩⟨ k ⟩ʷ B) →  Γ / lε ⊢ A :⇒*: B →  Γ / lε ⊩⟨ k ⟩ʷ B ≡ A / [B]
reflEqAuxʷ (N , [B]) D = (N , λ ≤ε lε' N<s N<s' → reflEqAux ([B] ≤ε lε' N<s) (wfRed≤* ≤ε D))


-- escapeEq : ∀ {k A B} → ([A] : Γ / lε ⊩⟨ k ⟩ A)
--             → Γ / lε ⊩⟨ k ⟩ A ≡ B / [A]
--             → Γ / lε ⊢ A ≅ B
-- escapeEq (Uᵣ′ k′ k< ⊢Γ) A=B rewrite A=B = ≅-Urefl ⊢Γ
...
-- escapeEq {k = k} (Bᵣ′ W F G D ⊢F ⊢G A≡A [Fₙ] [F] [G] G-ext) A=B =
--   LogRel.escapeEqB k (logRelRec _) (Bᵣ _ _ D ⊢F ⊢G A≡A [Fₙ] [F] [G] G-ext) A=B
--   -- ≅-red (red D) D′ ⟦ W ⟧ₙ ⟦ W ⟧ₙ A≡B
-- escapeEq (emb 0<1 A) A≡B = escapeEq A A≡B

-- escapeEqʷ-aux : ∀ {l : LCon} {lε : ⊢ₗ l} {k A B} → (N L : Nat)
--               → ([A] : Γ / lε ⊩⟨ k ⟩ʷ A)
--               → (A≡B : Γ / lε ⊩⟨ k ⟩ʷ A ≡ B / [A])
--               → (proj₁ [A] - sizeₗ l PE.≡ N)
--               → (proj₁ A≡B - sizeₗ l PE.≡ L)
--               → Γ / lε ⊢ A ≅ B
-- escapeEqʷ-aux {l = l} 0 0 (M , [A]) (K , A≡B) e₁ e₂
--   with ≤₋-0⇒≤ {M} {sizeₗ l} e₁ with ≤₋-0⇒≤ {K} {sizeₗ l} e₂
-- escapeEqʷ-aux {lε = lε} 0 0 (M , [A]) (K , A≡B) e₁ e₂
--   | N<s | K<s = escapeEq ([A] (≤ₗ-id _) lε N<s) (A≡B (≤ₗ-id _) lε N<s K<s)
-- escapeEqʷ-aux {lε = lε} 0 (1+ L) (M , [A]) (K , A≡B) e₁ e₂ =
--   ≅-ϝ {nε = NewNotInLConNat _} (escapeEqʷ-aux 0 L (τTyLogʷ {lε = lε} {nε = NewNotInLConNat _} (M , [A])) {!!} {!!} {!!}) {!!}
-- escapeEqʷ-aux {lε = lε} (1+ N) L (M , [A]) (K , A≡B) e₁ e₂ =
--   ≅-ϝ {nε = NewNotInLConNat _} (escapeEqʷ-aux N {!!} (τTyLogʷ {lε = lε} {nε = NewNotInLConNat _} (M , [A])) {!!} (-suc {n = M} e₁) {!!})
--                              (escapeEqʷ-aux N {!!} (τTyLogʷ {lε = lε} {nε = NewNotInLConNat _} (M , [A])) {!!} (-suc {n = M} e₁) {!!})

-- escapeEqʷ : ∀ {k A B} → ([A] : Γ / lε ⊩⟨ k ⟩ʷ A)
--             → Γ / lε ⊩⟨ k ⟩ʷ A ≡ B / [A]
--             → Γ / lε ⊢ A ≅ B
-- escapeEqʷ [A] (A≡B , _) = {!!}           
...
-- escapeTerm (Bᵣ′ BΠ F G D ⊢F ⊢G A≡A [Fₙ] [F] [G] G-ext)
--                (Πₜ ⊢f f≡f [f] [f]₁) =
--   conv ⊢f (sym (subset* (red D)))
-- escapeTerm (Bᵣ′ BΣ F G D ⊢F ⊢G A≡A [Fₙ] [F] [G] G-ext)

...
thm2 : ∀ (t : Term 1)
       → (ε ∙ (Π ℕ ▹ Unit) ⊩ℕ t ∷ℕ)
       → PE.⊥
thm2 t (ℕₜ n t→n n≡n (zeroᵣ)) = {!!}
thm2 t (ℕₜ n t→n n≡n (sucᵣ k)) = {!!}
thm2 t (ℕₜ (var x) t→n n≡n (ne (neNfₜ (var x) (var a b) n~n))) = (lol x b)
thm2 t (ℕₜ n t→n n≡n (ne (neNfₜ ( ∘ₙ f) KKK n~n))) = {!!}
thm2 t (ℕₜ n t→n n≡n (ne (neNfₜ nen [n] n~n))) = {!!}
